...
gene_term_old = pd.read_csv("raw data/gene_term_old.csv", dtype=object)
gene_term_old.columns = ["Gene","Term"]
len(gene_term_old), len(gene_term_old.Term.unique())

gene_term = pd.merge(gene_term_new, gene_term_old, how ='outer', on =['Gene', 'Term'])
gene_term.to_csv("data/gene_term_raw.csv", index=False)
print(gene_term)
